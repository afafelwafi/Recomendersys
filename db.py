...
def get_ids_form_url(liste,storeId):
    query="SELECT product_id FROM products WHERE store_id=%s AND picture_url IN ("+'%s,'*(len(liste)-1)+'%s)'
    cursor.execute(query,(tuple([storeId]+liste)))
    return cursor.fetchall()


def get_picture_url(liste,storeId):
    query="SELECT picture_url FROM products WHERE store_id=%s AND product_id IN ("+'%s,'*(len(liste)-1)+'%s)'
    cursor.execute(query,(tuple([storeId]+liste)))
    return cursor.fetchall()
